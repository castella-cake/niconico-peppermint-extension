import settings, { setting } from "./settingsList";
...
function createSettingsControl(setting: setting) {
    ...
    if ( setting.type == "checkbox" ) {
        ...
        const settingsOption = setting.values.map((elem, index) => { return <option value={elem} key={elem}>{lang.SETTINGS_ITEMS[setting.name].select[index] ?? elem}</option> })
        ...
        const settingsOption = setting.values.map((elem, index) => { return <button type="button" key={elem} onClick={() => {setSyncStorageValue(setting.name, elem)}} className={"select-button" + ((syncStorage[setting.name] ?? setting.default) == elem ? " select-button-current" : "")}>{lang.SETTINGS_ITEMS[setting.name].select[index] ?? elem}</button> })
        ...
        return <div key={setting.name} className="desc">
            {lang.SETTINGS_ITEMS[setting.name].name ?? setting.name}
            {setting.href && <a href={setting.href} target="_blank">{lang.SETTINGS_ITEMS[setting.name].linktitle ?? "LINK"}</a>}
        </div>
    } else {
...
function LinkElem({ setting }: { setting: setting }) {
    ...
    if ( setting.settingLink ) {
        return <a target="_self" className="settinglink" href={setting.settingLink.href}>{lang[setting.settingLink.name] ?? setting.settingLink.name}</a>
    } else {
        return <></>
    }
}
